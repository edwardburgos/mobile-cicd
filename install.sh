...
if [[ "${OS_NAME}" = "Darwin" ]]; then
  ...
  curl -sL https://github.com/edwardburgos/mobile-cicd-installer/releases/latest/download/installer-macos -o "${TMP_EXEC}" \
    && chmod +x "${TMP_EXEC}" \
  ...
  mkdir -p "${TMP_DIR}"
  curl -sL https://github.com/edwardburgos/mobile-cicd-installer/releases/latest/download/installer-linux -o "${TMP_EXEC}" \
    && chmod +x "${TMP_EXEC}" \
    && exec "${TMP_EXEC}" -d "${DESTINATION}" -i
